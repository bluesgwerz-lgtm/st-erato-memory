...
| 入库时机 | ~~每楼一次~~ → **v0.3 窗口制**：攒够 `autoInterval`（10）楼自动跑一次，一次调用最多 `windowFloors`（20）楼，按事件切条目；手动「总结到当前」不看间隔（见 §15） |
...
| 设置 | 含义 | 默认 | 取值 |
|---|---|---|---|
| `autoInterval` 自动总结间隔 | 攒够这么多 depth≥2 的未入库楼自动跑一次 | 10 | 0 = 手动；快捷 10/15/20；可手输任意 |
| `windowFloors` 单次总结最大楼层 | 一次副 API 调用最多吃多少楼 | 20 | 快捷 20/30/50/100；可手输任意 |
...
- W 越大越省钱也越粗（100 楼压成一次调用副模型只会给十几条事件）：日常 20–30，清不在乎细节的老积压再开 50–100。
